...
procedure TPenjualan8.btn6Click(Sender: TObject);
begin
edt1.Enabled:= True;
edt2.Enabled:= True;
edt3.Enabled:= True;
...
procedure TPenjualan8.btn1Click(Sender: TObject);
begin
// Validasi input
  if edt1.Text = '' then
  begin
  ...
  if edt4.Text = '' then
  begin
    ShowMessage('Kembali tidak boleh kosong!');
    Exit;
  end;
  if edt6.Text = '' then
  begin
  ...
  // Simpan data
  with DataModule4.tbl_penjualan do
  begin
    SQL.Clear;
    SQL.Add('INSERT INTO penjualan (invoice, total, bayar, kembali, tanggal, kustomer_id, user_id) VALUES ('
      + QuotedStr(edt1.Text) + ', '
      + QuotedStr(edt2.Text) + ', '
      + QuotedStr(edt3.Text) + ', '
      + QuotedStr(edt4.Text) + ', '
...
procedure TPenjualan8.btn2Click(Sender: TObject);
begin
// Validasi input
  if edt1.Text = '' then
  begin
...
procedure TPenjualan8.btn3Click(Sender: TObject);
begin
if MessageDlg('Apakah Anda yakin menghapus data ini?', mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    ...
    begin
      SQL.Clear;
      SQL.Add('DELETE FROM penjualan WHERE id = ' + QuotedStr(b)); // Assuming RecordID contains the record ID to delete
      ExecSQL;
...
procedure TPenjualan8.FormShow(Sender: TObject);
begin
posisiawal;
end;
...
procedure TPenjualan8.posisiawal;
begin
edt1.Enabled:= False;
edt2.Enabled:= False;
edt3.Enabled:= False;
...
procedure TPenjualan8.edt10Change(Sender: TObject);
begin
with DataModule4.tbl_penjualan do
begin
SQL.Clear;
SQL.Add('select * from penjualan where invoice like "%'+edt10.Text+'%"');
Open;
end;
end;

procedure TPenjualan8.btn7Click(Sender: TObject);
begin
DataModule4.ReportJual.ShowReport();
end;
